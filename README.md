 ...
 **둘은 @RestController와 @Controller의 차이처럼 HTTP Response Body가 생성되는 방식에 대한 차이가 있습니다!!!!**
 ...
 ## Q7. 테스트 코드를 작성해 볼까요?
> 테스트 코드를 작성하면, 관리해야 할 코드의 양이 늘어나는 것은 맞다. 하지만 테스트 코드를 유지함으로써 얻는 이점이 더 많다 !!!!
### 테스트 코드 작성의 장점
1. 개발 중 예상치 못한 문제를 미리 발견할 수 있다 (에러를 클라이언트 보다 빨리 발견 할 수 있다.)<br/> 
2. 작성한 코드가 의도한 대로 작동하는지 검증한다<br/> 
3. 변경에 대한 사이드 이펙트를 줄여준다<br/> 
4. 수정에서 유연하고 안정적이게 대응할 수 있다<br/> 
5. 변경 부분의 영향도를 쉽게 파악 가능하다<br/> 
6. 리팩토링 시 기능 구현이 동일하게 되었다는 판단의 기준이 된다 <br/>(예를 들어 버전업을 하면서 현재 구현된 기능이 제대로 버전업에서도 동일하게 구현되게 한다.)<br/> 
7. 동작하는 방식 및 결과 확인이 가능하다<br/> 
